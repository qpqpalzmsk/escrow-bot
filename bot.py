...
TRON_PASSWORD = os.getenv("TRON_PASSWORD", "")     # 단순 예시
...
REGISTERED_USERS = set()  # 전체 공지(/post) 보낼 때 사용

# ==============================
# 모든 메시지 → register_user
...
async def exit_to_start(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("대화를 취소합니다. /start 명령어로 다시 시작.\n" + command_guide())
...
async def set_item_name(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("상품 가격(USDT)을 입력 (숫자)\n(취소: /exit)" + command_guide())
...
async def set_item_type(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        await update.message.reply_text("상품 등록 중 오류가 발생.\n" + command_guide())
    finally:
        session.close()
    return ConversationHandler.END
...
# (동일하게 /cancel, /rate, /refund 등도 ConversationHandlers로 구현)
...
async def list_items_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text("상품 목록 조회 오류.\n" + command_guide())
    finally:
        session.close()
    ...
    try:
        ...
        try:
            await context.bot.send_message(
                chat_id=seller_id,
                text=(
                    f"당신의 상품 '{item.name}'에 거래 요청 도착.\n거래 ID: {t_id}\n"
                    "/accept 거래ID 판매자지갑주소 로 수락, /refusal 거래ID 로 거절.\n"
                    "※ TRC20 USDT"
                )
            )
        except Exception as e:
...
async def cancel(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        msg = f"취소 가능한 상품 (페이지 {page}/{total_pages}):\n"
        for idx, it in enumerate(page_items, start=1):
            msg += f"{idx}. {it.name} - {it.price} USDT ({it.type})\n"
        msg += "\n/next, /prev 로 페이지 이동\n취소할 상품 번호/이름 입력.\n(취소: /exit)"
        await update.message.reply_text(msg + command_guide())
...
async def cancel_item(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if identifier.lower() in ["/exit", "exit"]:
            return await exit_to_start(update, context)
        seller_id = update.message.from_user.id
...
async def accept_transaction(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        tx.status = "accepted"
        session.commit()
        await update.message.reply_text(f"거래 ID {t_id}가 수락됨.\n구매자에게 송금 안내.\n" + command_guide())
        try:
            await context.bot.send_message(
                chat_id=tx.buyer_id,
                text=(
                    f"거래 ID {t_id} 수락됨.\n"
                    f"{tx.amount} USDT를 {TRON_WALLET} 로 송금 시, 반드시 메모(거래ID:{t_id}) 기입."
                )
            )
        except Exception as e:
...
async def refusal_transaction(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        session.delete(tx)
        session.commit()
        await update.message.reply_text(f"거래 ID {t_id}가 거절됨.\n" + command_guide())
        try:
            await context.bot.send_message(
                chat_id=tx.buyer_id,
                text=f"거래 제안이 거절되었습니다. (거래ID {t_id})"
            )
        except Exception as e:
...
async def check_deposit(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not valid:
            await update.message.reply_text("입금 확인 실패 (금액 불일치/메모 누락 등).\n" + command_guide())
            return
        tx.status = "deposit_confirmed"
        session.commit()
        await update.message.reply_text("입금이 확인되었습니다! 판매자에게 안내.\n" + command_guide())
        try:
            await context.bot.send_message(
                chat_id=tx.seller_id,
                text=(
                    f"거래 ID {t_id} 입금 확인됨.\n물품 발송 후, 구매자가 /confirm 으로 완료 예정."
                )
            )
        except Exception as e:
            logging.error(f"판매자 알림 오류: {e}")
...
async def confirm_payment(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not tx:
            await update.message.reply_text("아직 입금확인 안 됐거나 거래 상태가 올바르지 않음.\n" + command_guide())
            return
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    f"거래 ID {t_id} 최종 완료됨.\n"
                    f"{net_amount} USDT 판매자 지갑({seller_wallet})으로 송금.\n"
                    f"송금 결과: {result}"
                )
            )
        except Exception as e:
...
async def refund_request(update: Update, context: CallbackContext) -> int:
    ...
    try:
        tx = session.query(Transaction).filter_by(transaction_id=t_id, status="deposit_confirmed").first()
        if not tx:
            await update.message.reply_text("유효한 거래 ID가 아니거나 환불 불가능.\n" + command_guide())
            return ConversationHandler.END
        if update.message.from_user.id != tx.buyer_id:
...
async def rate_user(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if not tx:
            await update.message.reply_text("완료된 거래가 아니거나 유효하지 않음.\n" + command_guide())
            return WAITING_FOR_RATING
...
async def save_rating(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if score < 1 or score > 5:
            await update.message.reply_text("평점은 1~5 사이여야 합니다.\n" + command_guide())
            return WAITING_FOR_CONFIRMATION
...
async def start_chat(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not tx:
            await update.message.reply_text("유효한 거래 ID가 아니거나 입금 미확인.\n" + command_guide())
            return
        user_id = update.message.from_user.id
        if user_id not in [tx.buyer_id, tx.seller_id]:
            await update.message.reply_text("거래 당사자가 아님.\n" + command_guide())
            return
        ...
        await update.message.reply_text("채팅 시작 중 오류.\n" + command_guide())
    finally:
        session.close()
...
async def relay_message(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        logging.error(f"채팅 메시지 중계 오류: {e}")
...
async def off_transaction(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if tx.status not in ["pending", "accepted", "deposit_confirmed", "deposit_confirmed_over"]:
            await update.message.reply_text("이미 완료되었거나 중단 불가능 상태.\n" + command_guide())
            return
        if update.message.from_user.id not in [tx.buyer_id, tx.seller_id]:
            await update.message.reply_text("해당 거래 당사자가 아님.\n" + command_guide())
            return
        ...
        await update.message.reply_text("거래 중단 처리 중 오류.\n" + command_guide())
    finally:
        session.close()
...
async def warexit_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if t_id in active_chats:
            active_chats.pop(t_id)
        await update.message.reply_text(f"[관리자] 거래 ID {t_id}가 강제 종료됨.\n" + command_guide())
    except Exception as e:
        session.rollback()
        logging.error(f"/warexit 오류: {e}")
        await update.message.reply_text("강제 종료 처리 중 오류.\n" + command_guide())
    finally:
        session.close()
...
def main():
    if not TELEGRAM_API_KEY:
        logging.error("TELEGRAM_API_KEY가 설정되지 않아 봇 실행 불가.")
        return
    ...
    # 6) ConversationHandlers (sell, cancel, rate, refund)
