...
async def offer_item(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            f"'{item.name}' 거래 요청이 생성되었습니다!\n거래 ID: {t_id}\n※ 송금 시 메모(거래 ID) 꼭 입력해주세요." + command_guide()
        )
        try:
...
async def confirm_payment(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        valid, _ = verify_deposit(float(tx.amount), txid, t_id)
        if not valid:
            await update.message.reply_text("TXID가 일치하지 않습니다. 다시 확인해주세요." + command_guide())
            return
        net_amount = (original_amount - 0.1) * (1 - NORMAL_COMMISSION_RATE)  # 예시로 NETWORK_FEE=0.1 etc.
        tx.status = "completed"
        session.commit()
        await update.message.reply_text(
...
async def refund_request(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        # etc... (구현)
        # 임의로 환불_amount계산 후
        refund_amount = float(tx.amount) * 0.975  # 예시 2.5% fee
...
async def start_chat(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text("채팅을 시작합니다. 텍스트/파일 전송 시 상대방에게 전달됩니다." + command_guide())
    except Exception as e:
...
def main():
    if not TELEGRAM_API_KEY:
        logging.error("TELEGRAM_API_KEY is empty! Check secrets.")
        return
